...
- **盈利回撤保护**：当收益>5%且从最高点回撤≥40%时自动平仓（每分钟检查）
...
### 🗣️ 自然语言交易（Telegram Bot）
- **AI 智能指令解析**：支持自然语言下单，如 "做多 BTC 2倍 100U" 或 "long ETH 3x $50"
- **多语言支持**：中英文指令识别
- **智能资产识别**：自动识别加密货币、股票、外汇、商品类型
- **置信度验证**：仅执行解析置信度 ≥80% 的指令
- **交易确认**：执行前交互式确认
